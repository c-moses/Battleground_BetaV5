class BGLayer1 extends BGBaseLayer // Chernarus Full
{
    ...
    ref array<Man> m_allPlayersWithin300m;
    ref array<Man> m_playersWithin100m;
    ref array<Man> m_playersWithin200m;
    ref array<Man> m_playersWithin300mOnly;

    override void InitLayer()
    {
        super.InitLayer();

        m_allPlayersWithin300m = new array<Man>;
        m_playersWithin100m = new array<Man>;
        m_playersWithin200m = new array<Man>;
        m_playersWithin300mOnly = new array<Man>;
    ...
    {
        m_allPlayersWithin300m.Clear();
        m_playersWithin100m.Clear();
        m_playersWithin200m.Clear();
        m_playersWithin300mOnly.Clear();

        array<Man> m_allPlayersWithin300m = GetPlayersInRadius(m_EventLocation, 300);
        
        foreach (Man player : m_allPlayersWithin300m)
        {
            float distanceToEvent = vector.Distance(player.GetPosition(), m_EventLocation);
            
            if (distanceToEvent <= 100.0)
            {
                m_playersWithin100m.Insert(player);
            }
            else if (distanceToEvent <= 200.0)
            {
                m_playersWithin200m.Insert(player);
            }
        }

        foreach (Man playerMan : m_allPlayersWithin300m)
        {
            if (m_playersWithin100m.Find(playerMan) == -1 && m_playersWithin200m.Find(playerMan) == -1)
            {
                m_playersWithin300mOnly.Insert(playerMan);
            }
        }

        ProcessPlayersInZone(m_playersWithin100m, 100);
        ProcessPlayersInZone(m_playersWithin200m, 50);
        ProcessPlayersInZone(m_playersWithin300mOnly, 20);
    }
